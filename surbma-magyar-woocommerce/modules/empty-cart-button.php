...
$emptycartbutton_cartpageValue = $cps_hc_gems_options['emptycartbutton-cartpage'] ?? 'none';
$emptycartbutton_checkoutpageValue = $cps_hc_gems_options['emptycartbutton-checkoutpage'] ?? 'none';
...
if ( 'none' != $emptycartbutton_cartpageValue ) :
	...
	add_action( $emptycartbutton_cartpageValue, static function() {
		if ( count( WC()->cart->get_cart() ) > 1 ) {
			...
			// Get the settings
			$emptycartbutton_cartpagebuttontextValue = $cps_hc_gems_options['emptycartbutton-cartpagebuttontext'] ?? __( 'Empty cart', 'surbma-magyar-woocommerce' );
			$emptycartbutton_checkoutpageconfirmationtextValue = $cps_hc_gems_options['emptycartbutton-confirmationtext'] ?? __( 'Are you sure you want to empty the Cart?', 'surbma-magyar-woocommerce' );
...
if ( 'none' != $emptycartbutton_checkoutpageValue ) :
	...
	add_action( $emptycartbutton_checkoutpageValue, static function() {
		if ( count( WC()->cart->get_cart() ) > 1 ) {
			...
			// Get the settings
			$emptycartbutton_checkoutpagemessageValue = $cps_hc_gems_options['emptycartbutton-checkoutpagemessage'] ?? __( 'Changed your mind?', 'surbma-magyar-woocommerce' );
			$emptycartbutton_checkoutpagelinktextValue = $cps_hc_gems_options['emptycartbutton-checkoutpagelinktext'] ?? __( 'Empty cart & continue shopping', 'surbma-magyar-woocommerce' );
			$emptycartbutton_checkoutpageconfirmationtextValue = $cps_hc_gems_options['emptycartbutton-confirmationtext'] ?? __( 'Are you sure you want to empty the Cart?', 'surbma-magyar-woocommerce' );
